...
temp_root="$(mktemp -d /tmp/tcc-profile-domain.XXXXXX)"
temp_project="$temp_root/project"
test_log="$temp_root/profile-domain-tests.log"
hub_licensing_ipc="$(
  ps -ax -o command= |
...
cleanup() {
  ...
  if [[ $exit_code -ne 0 && -f "$test_log" ]]; then
    print -u2 "Profile domain tests failed. Relevant output:"
    grep -E '\[TCC Profile\]|error CS|Exception|Error' "$test_log" | tail -n 100 >&2 || true
  fi
  rm -rf "$temp_root"
}
trap cleanup EXIT
...
"$unity_editor" \
  ...
  -executeMethod TCC.EditorTools.ProfileDomainValidation.RunBatch \
  -logFile "$test_log"

grep -F "[TCC Profile] PASS:" "$test_log"
