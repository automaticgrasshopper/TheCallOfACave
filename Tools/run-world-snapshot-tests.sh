...
temp_root="$(mktemp -d /tmp/tcc-world-snapshot.XXXXXX)"
temp_project="$temp_root/project"
test_log="$temp_root/world-snapshot-tests.log"
hub_licensing_ipc="$(
  ps -ax -o command= |
...
cleanup() {
  ...
  if [[ $exit_code -ne 0 && -f "$test_log" ]]; then
    print -u2 "World snapshot tests failed. Relevant output:"
    grep -E '\[TCC Snapshot\]|error CS|Exception|Error' "$test_log" | tail -n 120 >&2 || true
  fi
  rm -rf "$temp_root"
}
trap cleanup EXIT
...
"$unity_editor" \
  ...
  -executeMethod TCC.EditorTools.WorldSnapshotValidation.RunBatch \
  -logFile "$test_log"

grep -F "[TCC Snapshot] PASS:" "$test_log"
