...
temp_root="$(mktemp -d /tmp/tcc-profile-migration.XXXXXX)"
temp_project="$temp_root/project"
test_log="$temp_root/profile-migration-tests.log"
hub_licensing_ipc="$(
  ps -ax -o command= |
...
cleanup() {
  ...
  if [[ $exit_code -ne 0 && -f "$test_log" ]]; then
    print -u2 "Profile migration tests failed. Relevant output:"
    grep -E '\[TCC Migration\]|error CS|Exception|Error' "$test_log" | tail -n 140 >&2 || true
  fi
  rm -rf "$temp_root"
}
trap cleanup EXIT
...
"$unity_editor" \
  ...
  -executeMethod TCC.EditorTools.ProfileMigrationValidation.RunBatch \
  -logFile "$test_log"

grep -F "[TCC Migration] PASS:" "$test_log"
